-- Idempotent MySQL migration for database-backed, per-town shop inventories.
-- Run this against the same database used by the Unity client (currently `accounts`).
...
-- Replace the old all-items-at-every-town starter stock for these shops.
DELETE FROM shop_stock
WHERE node_id IN ('nodeFortAurus', 'nodeMounttown', 'nodeRiverVillage', 'nodeSmallVillage');
...
SELECT nodes.name AS town, items.name AS item, shop_stock.price
FROM shop_stock
JOIN nodes ON nodes.id = shop_stock.node_id
JOIN items ON items.id = shop_stock.item_id
WHERE shop_stock.node_id IN ('nodeFortAurus', 'nodeMounttown', 'nodeRiverVillage', 'nodeSmallVillage')
ORDER BY nodes.name, shop_stock.price, items.name;
